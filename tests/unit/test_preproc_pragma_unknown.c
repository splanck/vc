...
int main(void)
{
    ...
    int fd = mkstemp(tmpl);
    ASSERT(fd >= 0);
    const char *src = "#pragma unknown_directive\nint x;\n";
    if (fd >= 0) {
    ...
    ASSERT(res != NULL);
    if (res) {
        ASSERT(strstr(res, "#pragma unknown_directive") != NULL);
        ASSERT(strstr(res, "int x;") != NULL);
    }
    free(res);
    ...
    if (failures == 0)
        printf("All preproc_pragma_unknown tests passed\n");
    else
        printf("%d preproc_pragma_unknown test(s) failed\n", failures);
    return failures ? 1 : 0;
}
