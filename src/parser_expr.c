...
static expr_t *clone_expr(const expr_t *expr)
{
    ...
    case EXPR_CALL: {
        ...
        expr_t *fn = clone_expr(expr->call.func);
        if (!fn) {
            for (size_t i = 0; i < n; i++)
        ...
        return ast_make_call(fn, args, n,
                             expr->line, expr->column);
    }
    }
    return NULL;
...
static expr_t *parse_primary(parser_t *p)
{
    ...
    while (1) {
        if (match(p, TOK_LPAREN)) {
            ...
            if (!args) { ast_free_expr(base); return NULL; }
            if (!match(p, TOK_RPAREN)) {
                do {
                    expr_t *arg = parse_expression(p);
                    if (!arg) { for (size_t i=0;i<count;i++) ast_free_expr(args[i]); free(args); ast_free_expr(base); return NULL; }
                    if (count >= cap) {
                        cap *= 2; expr_t **tmp = realloc(args, cap*sizeof(*tmp));
                        if (!tmp) { for (size_t i=0;i<count;i++) ast_free_expr(args[i]); free(args); ast_free_expr(base); return NULL; }
                        args = tmp;
                    }
                    args[count++] = arg;
                } while (match(p, TOK_COMMA));
                if (!match(p, TOK_RPAREN)) { for (size_t i=0;i<count;i++) ast_free_expr(args[i]); free(args); ast_free_expr(base); return NULL; }
            }
            base = ast_make_call(base, args, count, lp->line, lp->column);
